...
Z=await (async function(){function a(c){var d=Z=c.exports;c={};for(let [e,g]of Object.entries(d))"function"==typeof g?(d=qa(g),c[e]=d):c[e]=g;c=Z=c;Ja=f._main=c.u;Fa=c.v;f._chessiro_command=c.w;Ka=c.x;za=c.y;Ha=c.z;S=c.A;Ia=c.B;dynCall_vii=c.D;dynCall_vi=c.G;dynCall_iii=c.H;dynCall_v=c.L;Da=c.X;ra=c.Y;Ca=c.Z;Ea=c._;J=c.s;fa();return Z}var b={a:La};if(f.instantiateWasm)return new Promise(c=>{f.instantiateWasm(b,(d,e)=>{c(a(d,e))})});L??=f.locateFile?f.locateFile("sf19-st-relaxed.wasm",w):
w+"sf19-st-relaxed.wasm";return a((await ja(b)).instance)}());(function(a=r){f.calledRun=!0;E||(ea=!0,Z.t(),F?.(f),Ma(a))})();ea?moduleRtn=f:moduleRtn=new Promise((a,b)=>{F=a;ba=b});
